...
const S_users = require("../../controller/system/login.js");
  ...
  if(yzmsign== ctx.session.captcha){
    ...
    if(R_code==0 && R_success==true){
        ...
        let T_resData=  await  S_users.user_TokenAdd([
          R_data.id,identity_type,user_randomStr,user_ip
        ]);
        ...
        let enTokenStr=  tokenutil.enToken({
          ...
          uid:R_data.id,
          uname:R_data.username,
          lname:R_data.loginname,
        ...
        console.log("登录信息记录")
        // console.log(data)
        console.log({code:0,success:true,msg:"登录成功",data:{uid:R_data.id,utype:identity_type}})
        
         ctx.body= {code:0,success:true,msg:"登录成功",data:{uid:R_data.id,utype:identity_type}};
        }else{
