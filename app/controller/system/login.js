...
async function  user_login(option){
    let rows= await exec(" call p_user_login(?,?,?) ",option)
...
async function  user_TokenAdd(option){
    let rows= await exec(" insert into user_token(uid,identity_type,logintime,randomkey,ip) value(?,?,now(),?,?)",option)
...
async function  user_Token(option){
    let sql=`call p_user_token("${option[0]}","${option[1]}","${option[2]}","${option[3]}")`;
    //  console.log(sql)
    let rows= await  exec(sql,option)
    if(rows[0][0].res>0){
        return true;
      }else{
        return false;
...
async function  user_userinfo(option){
    let rows= await exec(" call p_user_userinfo(?,?)",option)
    if(rows[0].length>=1){
        return rows[0][0];
    }else{
        return null;
...
async function  user_Uregister(option){
    let rows= await exec("insert into `user`(uid,nick_name,gender,birthday,face,register_source,createtime,status) value(?,?,?,?,?,?,now(),?) ",option)
    if(rows.affectedRows>0){
        return {code:2,success:true,msg:"注册成功",data:rows};
    }else{
        return {code:3,success:false,msg:"注册失败",data:null};
...
async function  user_UAregister(option){
    let rows= await exec(" insert into `user_auths`(uid,identity_type,identifier,credential,createtime,ip,status,extend1,extend2,extend3) value(?,?,?,?,now(),?,?,null,null,null) ",option)
    if(rows.affectedRows>0){
        return {code:2,success:true,msg:"注册成功",data:rows};
    }else{
        return {code:3,success:false,msg:"注册失败",data:null};
...
async function  user_register_verify(option){
    let rows= await exec(" call p_user_register_verify(?,?)",option)
    try {
        if(rows[0].length==0){
            return true;
          }else{
