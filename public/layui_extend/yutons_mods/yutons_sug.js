...
layui.define(['jquery', 'table'], function(exports) {
	...
	yutons_sug.prototype.render = function(opt) {
		...
		// opt.fieldecho=opt.fieldecho || []; ///设置字段映射，适用于输入一个字段，回显多个字段
		if(opt.divwidth!=null && opt.divwidth!=undefined){

		}else{
			divwidth="100%";
		}
		//初始化输入框提示容器
		$("#" + opt.id).after("<div id='sugItem' style='background-color: #fff;display: none;z-index:1;position: absolute;width:"+opt.divwidth+";'></div>");
	...
	function getSugTable(opt) {
		...
		table.on('rowDouble(' + "yutons_sug_" + opt.id + ')', function(obj) {
           ...
           var fieldecho=opt.fieldecho;
		   if(fieldecho.length>0){
			for (let p in fieldecho) {
				$("#" + fieldecho[p]).val(obj.data[fieldecho[p]]);
			}	
			$("#" + opt.id).next().hide().html("");
		   }else{
	            //获取选中行所传入字段的值
	            $("#" + opt.id).val(obj.data[opt.id]);
	            // $("#" + opt.).val(obj.data[opt.id]);
	            $("#" + opt.id).next().hide().html("");
		   }
